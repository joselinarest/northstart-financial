...
import {kidsGoalSummary,monthlyHoldingPlan,monthlyCashAction} from '../lib/kids-plan-summary.ts';
...
const deficits=[{ticker:"A",remaining:20000},{ticker:"B",remaining:15000}];
assert.equal(monthlyCashAction(35000,0,0,deficits).deposit,35000);
assert.equal(monthlyCashAction(35000,10000,5000,deficits).deposit,20000);
assert.equal(monthlyCashAction(35000,10000,30000,deficits).deposit,0);
assert.equal(monthlyCashAction(35000,0,null,deficits).deposit,null);
assert.equal(monthlyCashAction(35000,40000,0,deficits).remaining,0);
assert.equal(monthlyCashAction(35000,10000,0,deficits).allocations.reduce((sum,r)=>sum+r.planned,0),25000);
console.log("Cash action passed: deducts existing cash and purchases, unknown cash stays unknown, per-holding plan stays within remaining budget.");
