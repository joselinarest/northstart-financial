...
const {marketSessionAt}=await import('../work/calendar-test/market-session.js'),{exchangeDay}=await import('../work/calendar-test/exchange-calendar.js'),{selectSnapshotPrice,completedBarTimestamp}=await import('../work/calendar-test/providers/alpaca-market-data.js');
...
assert.deepEqual(selectSnapshotPrice(snapshot,'CLOSED',Date.parse('2026-09-26T06:00:00Z')),{last:100,timestamp:'2026-09-25T20:00:00.000Z'});assert.deepEqual(selectSnapshotPrice(snapshot,'REGULAR'),{last:105,timestamp:'2026-09-25T20:30:00Z'});
console.log('PASS holidays, early closes, DST, calendar expiry, last-close selection and price timestamp integrity');

assert.equal(completedBarTimestamp('2026-11-27T05:00:00Z',Date.parse('2026-11-28T06:00:00Z')),'2026-11-27T18:00:00.000Z');
assert.equal(completedBarTimestamp('2026-01-20T05:00:00Z',Date.parse('2026-01-21T06:00:00Z')),'2026-01-20T21:00:00.000Z');
assert.equal(completedBarTimestamp('2026-09-25T04:00:00Z',Date.parse('2026-09-25T18:00:00Z')),'2026-09-25T04:00:00Z');
assert.equal(completedBarTimestamp('invalid'),null);
