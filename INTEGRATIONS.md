...
2. Create a Cognito managed-login domain. Set `NEXT_PUBLIC_COGNITO_DOMAIN` to its full HTTPS URL, and set both client-ID variables to the app client ID.
   In AWS Amplify Hosting, add `NEXT_PUBLIC_COGNITO_DOMAIN`, `NEXT_PUBLIC_COGNITO_CLIENT_ID`, `COGNITO_USER_POOL_ID`, and `COGNITO_ISSUER` under **Hosting → Environment variables** for the production branch. `COGNITO_ISSUER` has the form `https://cognito-idp.REGION.amazonaws.com/USER_POOL_ID`. The committed build specification copies only these non-secret Cognito identifiers into `.env.production` before `next build`; missing values stop deployment with a precise error instead of publishing authentication that fails only after login.
