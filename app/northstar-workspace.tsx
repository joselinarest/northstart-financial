...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string }) {
  ...
  useEffect(()=>{if(initialTab!=="Portfolio"||!advisorAccount)return;const text=`${advisorAccount.subtype||""} ${advisorAccount.name||""}`.toLowerCase();const type=text.includes("401")?"401(k)":text.includes("roth")?"Roth IRA":text.includes("ira")?"Traditional IRA":"Taxable brokerage";setPortfolioAccount(type)},[initialTab,advisorAccount]);
  ...
  const swingDecisionTabs=["Daily Action Plan","Prepare Trade"],longTermInvestmentTabs=["Portfolio","Growth Finder"],isSwingDecisionPage=swingDecisionTabs.includes(initialTab),isLongTermInvestmentPage=longTermInvestmentTabs.includes(initialTab);
  ...
  useEffect(()=>{if(["Dashboard","Accounts","Account Transactions","Portfolio","Market Intel","Prepare Trade","Professional Charts","Household","Bills & cards","Liabilities"].includes(initialTab)&&signedIn)loadConnectedFinance()},[initialTab,signedIn,accessToken]);
  useEffect(()=>{if(!signedIn)return;if(initialTab==="Household")loadHousehold();if(["Dashboard","Accounts","Account Transactions","Portfolio","Market Intel","Prepare Trade","Professional Charts","Household","Bills & cards","Liabilities"].includes(initialTab))loadConnectedFinance()},[realtimeTick]);
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container">
          ...
          {tab==="Liabilities"&&<DebtLiabilityCenter accounts={connectedFinance.accounts}/>}
          ...
          {tab==="Accounts"&&<InvestmentAccountManager accessToken={accessToken} onChanged={()=>{loadConnectedFinance();setRealtimeTick(value=>value+1)}}/>}
          ...
          <section className="portfolio-builder card">
            {tab==="Portfolio"&&<ActionGuidancePanel accountId={advisorAccountId} accessToken={accessToken} mode="month"/>}
