...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "", initialBillName = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string; initialBillName?:string }) {
  ...
  const openAccountTransactions=async(accountId:string)=>{setSelectedFinanceAccountId(accountId);setAccountTransactions([]);setAccountTransactionsStatus("Loading all transactions for this account…");try{const response=await fetch(`/api/transactions?accountId=${encodeURIComponent(accountId)}`,{headers:financeHeaders(),cache:"no-store"}),data=await apiPayload(response);if(!response.ok)throw new Error(data.error||"Transactions could not be loaded");const rows=Array.isArray(data)?data:[];setAccountTransactions(rows);setAccountTransactionsStatus(rows.length?`${rows.length.toLocaleString()} transactions loaded`:`No transactions have been synchronized for this account.`)}catch(error){setAccountTransactionsStatus(error instanceof Error?error.message:"Transactions could not be loaded")}};
  useEffect(()=>{
