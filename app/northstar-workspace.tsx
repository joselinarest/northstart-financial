...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean }) {
  ...
  useEffect(()=>{let active=true;const load=()=>fetch("/api/market/clock").then(response=>response.json()).then(data=>{if(active)setMarketClock(data)}).catch(()=>{if(active)setMarketClock({status:"temporarily_unavailable",configured:true,error:"The market-clock service could not be reached."})});load();const refresh=window.setInterval(load,60000),tick=window.setInterval(()=>setClockTick(Date.now()),30000);return()=>{active=false;window.clearInterval(refresh);window.clearInterval(tick)}},[]);
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {tab==="Prepare Trade"&&advisorHoldings.length>0&&<ConnectedHoldingsAnalysis holdings={advisorHoldings} mode={advisorStrategy} accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
          {tab==="Accounts"&&<section className="connected-accounts card">
            ...
            {tab==="Accounts"&&swingHoldings.length>0&&<ConnectedHoldingsAnalysis holdings={swingHoldings} mode="swing" accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
            {tab==="Accounts"&&longTermHoldings.length>0&&<ConnectedHoldingsAnalysis holdings={longTermHoldings} mode="long-term" accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
