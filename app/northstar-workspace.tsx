...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean }) {
  ...
  useEffect(()=>{if(["Dashboard","Accounts","Portfolio","Market Intel","Prepare Trade","Professional Charts"].includes(initialTab)&&signedIn)loadConnectedFinance()},[initialTab,signedIn,accessToken]);
  useEffect(()=>{if(!signedIn)return;if(initialTab==="Household")loadHousehold();if(["Dashboard","Accounts","Portfolio","Market Intel","Prepare Trade","Professional Charts"].includes(initialTab))loadConnectedFinance()},[realtimeTick]);
