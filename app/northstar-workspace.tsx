    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {tab==="Daily Action Plan"&&<section className="daily-account-action-plans"><header><span>SWING / OPTIONS · ACCOUNT-BY-ACCOUNT GUIDANCE</span><h2>Today’s trading actions are separated by active trading account</h2><p>Each panel uses only that Swing or Options account’s positions, cash, risk limits, setups, and recommendation history. Long-term accounts are reviewed in Portfolio.</p></header>{swingAccounts.map((account,index)=>{const name=String(account.nickname||account.official_name||account.name||"Trading account"),holdingCount=connectedFinance.holdings.filter(holding=>String(holding.account_id)===String(account.id)).length;return <details key={String(account.id)} open={index===0}><summary><span className="swing">SWING / OPTIONS</span><b>{name}</b><small>{account.investment_purpose||account.subtype||"Active trading"} · {holdingCount} holding{holdingCount===1?"":"s"}</small><i>Show / hide</i></summary><ActionGuidancePanel accountId={String(account.id)} accessToken={accessToken} mode="today"/></details>})}{!swingAccounts.length&&<div className="purpose-empty"><b>No Swing or Options account is configured.</b><span>Assign an investment account to Swing or Options. Retirement, Kid Growth, and other long-term accounts intentionally do not receive daily trading actions here.</span></div>}</section>}
