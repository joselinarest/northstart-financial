      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container" aria-busy={pageDataLoading}>
          ...
          {tab==="Accounts"&&<section className="connected-accounts card">
            ...
            {investmentAccounts.map(account=>{const accountHoldings=connectedFinance.holdings.filter(holding=>String(holding.account_id)===String(account.id)),accountMode=isSwingAccount(account)?"swing":"long-term";return <InvestmentAccountProfile key={`profile_${account.id}`} account={account} holdings={accountHoldings} onSave={updateInvestmentAccount} onAddHolding={addManualHolding}>{accountHoldings.length?<section className="account-evaluation-panel"><header><b>Holding evaluations</b><span>Open one holding at a time to inspect its evidence, risk, and suggested action.</span></header><ConnectedHoldingsAnalysis marketOpen={marketPhase==="open"} holdings={accountHoldings} mode={accountMode} accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-chart-account",String(account.id));setAdvisorAccountId(String(account.id));setChartSymbol(symbol);navigate("Professional Charts")}} /></section>:<div className="account-evaluation-empty">No holdings are available for evaluation in this account yet.</div>}</InvestmentAccountProfile>})}
          </section>}
