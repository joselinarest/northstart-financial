      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container" aria-busy={routeLoading}>
          ...
          {tab==="Accounts"&&<section className="connected-accounts card">
            ...
            {tab==="Accounts"&&investmentAccounts.map(account=>{const accountHoldings=connectedFinance.holdings.filter(holding=>String(holding.account_id)===String(account.id));if(!accountHoldings.length)return null;const accountValue=accountHoldings.reduce((sum,holding)=>sum+Number(holding.market_value_cents||0),0)/100,accountName=String(account.nickname||account.official_name||account.name||"Investment account"),accountMode=isSwingAccount(account)?"swing":"long-term";return <section className="account-holdings-group" key={`holding_evaluations_${account.id}`}><header><div><span>{accountMode==="swing"?"SWING ACCOUNT":"LONG-TERM ACCOUNT"} · OWNER: {String(account.owner_name||account.entity_name||"Household")}</span><h3>{accountName}</h3><p>{account.subtype||"Investment"} {account.mask?`•••• ${account.mask}`:""} · Every position and recommendation below belongs only to this account.</p></div><strong>${accountValue.toLocaleString(undefined,{minimumFractionDigits:2,maximumFractionDigits:2})}<small>{accountHoldings.length} holding{accountHoldings.length===1?"":"s"}</small></strong></header><ConnectedHoldingsAnalysis marketOpen={marketPhase==="open"} holdings={accountHoldings} mode={accountMode} accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-chart-account",String(account.id));setAdvisorAccountId(String(account.id));setChartSymbol(symbol);navigate("Professional Charts")}} /></section>})}
          </section>}
