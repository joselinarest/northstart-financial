...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "", initialBillName = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string; initialBillName?:string }) {
  ...
  const portfolioTrajectory=useMemo(()=>{const years=portfolioYears<=5?Array.from({length:portfolioYears+1},(_,year)=>year):Array.from({length:6},(_,index)=>Math.round(portfolioYears*index/5)),lowRate=Math.max(-.05,weightedReturn-.08),highRate=weightedReturn+.05,points=years.map(year=>({year,low:portfolioAmount*Math.pow(1+lowRate,year),base:portfolioAmount*Math.pow(1+weightedReturn,year),high:portfolioAmount*Math.pow(1+highRate,year)})),all=points.flatMap(point=>[point.low,point.base,point.high]),min=Math.min(...all),max=Math.max(...all),range=Math.max(1,max-min),path=(key:"low"|"base"|"high")=>points.map((point,index)=>`${(index/Math.max(1,points.length-1)*100).toFixed(1)},${(92-(point[key]-min)/range*82).toFixed(1)}`).join(" ");return{points,lowPath:path("low"),basePath:path("base"),highPath:path("high")}},[portfolioAmount,portfolioYears,weightedReturn]);
      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container" aria-busy={pageDataLoading}>
          ...
          <section className="portfolio-builder card">
            ...
            <PortfolioTrajectoryExplorer points={portfolioTrajectory.points} horizon={portfolioGoal==="5 years"?"3–5 years":portfolioGoal}/>
