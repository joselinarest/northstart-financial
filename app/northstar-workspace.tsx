...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "", initialBillName = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string; initialBillName?:string }) {
  ...
  const applyPortfolioPreset = () => {const nextMix=portfolioGoal==="2–3 years"?{cash:35,bonds:40,diversified:20,dividend:5,growth:0}:portfolioGoal==="10+ years"?{cash:5,bonds:10,diversified:55,dividend:15,growth:15}:{cash:10,bonds:10,diversified:40,dividend:20,growth:20},nextAmount=connectedPortfolioValue>0?connectedPortfolioValue:portfolioAmount;setPortfolioMix(nextMix);setPortfolioAmount(nextAmount);localStorage.setItem("northstar-portfolio-plan",JSON.stringify({advisorAccountId,advisorAccountName,portfolioGoal,portfolioAccount,portfolioAmount:nextAmount,portfolioMix:nextMix,analyzedAt:new Date().toISOString(),status:"smart-target-confirmed"}));setPortfolioNotice(`✓ Smart target analyzed, confirmed, and saved for ${advisorAccountName} at ${new Date().toLocaleString()}.`)};
  ...
  const savePortfolio = () => {if(portfolioTotal!==100){setPortfolioNotice(`Allocation totals ${portfolioTotal}%. Adjust it to exactly 100% before saving.`);return}localStorage.setItem("northstar-portfolio-plan",JSON.stringify({advisorAccountId,advisorAccountName,portfolioGoal,portfolioAccount,portfolioAmount,portfolioMix,analyzedAt:new Date().toISOString(),status:"confirmed"}));setPortfolioNotice(`✓ Account, horizon, amount, and target saved for ${advisorAccountName} at ${new Date().toLocaleString()}.`)};
