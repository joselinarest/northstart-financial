...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "", initialBillName = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string; initialBillName?:string }) {
  ...
  const pathByTab: Record<string,string> = { Dashboard:"dashboard", Accounts:"accounts", "Account Transactions":"account-transactions", "Bill Transactions":"bill-transactions", "Market Intel":"markets", Portfolio:"portfolio", "Real Estate":"real-estate", "Professional Charts":"charts", "Market News":"market-news", "Growth Finder":"growth", "New Candidates":"new-candidates", "Bills & cards":"cash-flow", "Daily Action Plan":"daily-action-plan", Liabilities:"debt", Household:"household", "Ask Northstar":"assistant", "Prepare Trade":"daily-action-plan", "Paper Simulator":"simulation", Journal:"journal", Learn:"academy", Settings:"settings", Help:"help" };
  ...
  const clearHeaderAlerts=async()=>{if(!headerAlerts.length&&marketUnread===0)return;await confirmAction({title:"Clear all notifications?",description:"This removes all current notifications from this household's Notification Center. Future alerts will continue to arrive.",confirmLabel:"Clear all",cancelLabel:"Keep notifications",variant:"warning",context:<span>{marketUnread} unread notification{marketUnread===1?"":"s"}</span>,onConfirm:async()=>{const response=await fetch("/api/alerts",{method:"PATCH",headers:financeHeaders(),body:JSON.stringify({clearAll:true})}),data=await response.json() as {error?:string};if(!response.ok)throw new Error(data.error||"Notifications could not be cleared");setHeaderAlerts([]);setMarketUnread(0)}})};
  ...
  const mobileActive=(name:string)=>name==="Market Intel"?marketPages.includes(tab):name==="Bills & cards"?["Accounts","Account Transactions","Bills & cards","Bill Transactions","Liabilities","Household","Real Estate"].includes(tab):tab===name;
        ...
        <section className="content page-container" aria-busy={pageDataLoading}>
          ...
          <nav className="workspace-breadcrumb" aria-label="Page navigation">
            ...
            <ol>
              ...
              {["Accounts","Account Transactions","Bills & cards","Bill Transactions","Liabilities","Household","Real Estate"].includes(tab)&&<li><button type="button" onClick={() => navigate("Household")}>Financial Life</button></li>}
          ...
          <div className="hero" id="dashboard-top">
            <div>
              ...
              <h1>{tab==="Dashboard"?"Home — What matters most right now?":tab==="Account Transactions"?"Account activity and cash-flow snapshot":tab==="Bill Transactions"?"Bill history and cost comparison":tab==="Portfolio"?"Portfolio — Is my money allocated correctly?":tab==="Daily Action Plan"?"Today — What should I prepare to do now?":tab==="Growth Finder"?"Long-Term — What should I accumulate, and why?":tab==="New Candidates"?"New Candidates — What is improving before broad recognition?":tab==="Professional Charts"?"Markets & Charts — What does current evidence show?":tab==="Market News"?"News — What actually matters to me today?":tab==="Real Estate"?"Real Estate — Is this property a good investment?":tab==="Household"?"Household — Is my financial foundation safe?":tab==="Learn"?"Academy — What do I need to learn next?":tab==="Ask Northstar"?"Advisor — What decision should we evaluate?":tab==="Paper Simulator"?"Practice — Can I apply the process without risking money?":"Your professional investment workspace."}</h1>
              <p>
