...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string }) {
  ...
  useEffect(()=>{if(["Dashboard","Accounts","Account Transactions","Portfolio","Market Intel","Prepare Trade","Professional Charts","Household","Bills & cards"].includes(initialTab)&&signedIn)loadConnectedFinance()},[initialTab,signedIn,accessToken]);
  useEffect(()=>{if(!signedIn)return;if(initialTab==="Household")loadHousehold();if(["Dashboard","Accounts","Account Transactions","Portfolio","Market Intel","Prepare Trade","Professional Charts","Household","Bills & cards"].includes(initialTab))loadConnectedFinance()},[realtimeTick]);
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {(tab==="Household"||tab==="Bills & cards")&&<HouseholdMoneyCenter accounts={connectedFinance.accounts} transactions={familyTransactions}/>}
