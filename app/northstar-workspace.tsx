...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string }) {
  ...
  const applyWorkspacePath=(pathname:string)=>{
  ...
  const navigatePath=(pathname:string,{replace=false}:{replace?:boolean}={})=>{
    ...
    setRouteLoading(true);applyWorkspacePath(destination.pathname);
  ...
  useEffect(()=>{
    const restoreRoute=()=>{setRouteLoading(true);applyWorkspacePath(window.location.pathname);window.setTimeout(()=>setRouteLoading(false),180)};
  ...
  useEffect(()=>{if(["Dashboard","Accounts","Account Transactions","Portfolio","Market Intel","Prepare Trade","Professional Charts","Household","Bills & cards","Liabilities"].includes(tab)&&signedIn)loadConnectedFinance()},[tab,signedIn,accessToken]);
  ...
  return (
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${researchDetailOpen?"page-research-detail":""}`}>
      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container" aria-busy={routeLoading}>
          ...
          {tab==="Real Estate"&&<RealEstateCommandCenter/>}
