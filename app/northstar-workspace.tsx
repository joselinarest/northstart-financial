...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean }) {
  ...
  const longTermAccounts=useMemo(()=>investmentAccounts.filter(account=>!["Swing","Options"].includes(String(account.investment_purpose||""))),[investmentAccounts]);
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {isSwingDecisionPage&&!swingAccounts.length&&<section className="purpose-empty card"><b>No Swing or Options investment account is configured.</b><span>Every page under Swing / Options Accounts is reserved for that account purpose. Open Accounts, assign Swing or Options, and return here.</span><button onClick={()=>navigate("Accounts")}>Configure swing account →</button></section>}
          ...
          {tab==="Daily Action Plan"&&<section className="daily-account-action-plans"><header><span>ACCOUNT-BY-ACCOUNT GUIDANCE</span><h2>Today’s actions are separated so capital and strategy never mix</h2><p>Each panel uses only that account’s holdings, cash, goal, allocation, risk limits, and recommendation history.</p></header>{[...swingAccounts,...longTermAccounts].map((account,index)=>{const isSwing=["Swing","Options"].includes(String(account.investment_purpose||"")),name=String(account.nickname||account.official_name||account.name||"Investment account"),holdingCount=connectedFinance.holdings.filter(holding=>String(holding.account_id)===String(account.id)).length;return <details key={String(account.id)} open={index===0}><summary><span className={isSwing?"swing":"long-term"}>{isSwing?"SWING / OPTIONS":"LONG-TERM"}</span><b>{name}</b><small>{account.investment_purpose||account.subtype||"Purpose not assigned"} · {holdingCount} holding{holdingCount===1?"":"s"}</small><i>Show / hide</i></summary><ActionGuidancePanel accountId={String(account.id)} accessToken={accessToken} mode="today"/></details>})}{!investmentAccounts.length&&<div className="purpose-empty"><b>No investment account is configured.</b><span>Create an account first so Today can calculate separate actions without mixing money or strategy.</span></div>}</section>}
