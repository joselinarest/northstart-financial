...
export function NorthstarWorkspace({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false, initialFinanceAccountId = "", initialBillName = "" }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean; initialFinanceAccountId?: string; initialBillName?:string }) {
  ...
  const swingDecisionTabs=["Daily Action Plan","Prepare Trade"],longTermInvestmentTabs=["Portfolio","Growth Finder"],isSwingDecisionPage=swingDecisionTabs.includes(tab),isLongTermInvestmentPage=longTermInvestmentTabs.includes(tab);
  ...
  useEffect(()=>{const required=isSwingDecisionPage?swingAccounts:isLongTermInvestmentPage?longTermAccounts:null;if(!required||!required.length||required.some(account=>String(account.id)===advisorAccountId))return;const next=String(required[0].id);setAdvisorAccountId(next);localStorage.setItem("northstar-advisor-account",next)},[isSwingDecisionPage,isLongTermInvestmentPage,swingAccounts,longTermAccounts,advisorAccountId]);
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${["Account Transactions","Bill Transactions"].includes(tab)?"page-route-detail":""} ${researchDetailOpen?"page-research-detail":""}`}>
      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container" aria-busy={pageDataLoading}>
          ...
          {tab==="Daily Action Plan"&&<section className="daily-account-action-plans selected-account-only"><header><span>SELECTED SWING / OPTIONS ACCOUNT</span><h2>{swingAdvisorName}</h2><p>{swingAdvisorAccount?`Every item below uses only this account’s ${swingAdvisorHoldings.length} holding${swingAdvisorHoldings.length===1?"":"s"}, cash, buying power, risk limits, and action history. Change accounts with the selector above.`:"Choose or configure a Swing/Options account. Long-term accounts are reviewed in Portfolio, not Today."}</p></header>{swingAdvisorAccount?<ActionGuidancePanel accountId={String(swingAdvisorAccount.id)} accessToken={accessToken} mode="today" marketOpen={marketPhase==="open"}/>:<div className="purpose-empty"><b>No Swing or Options account is configured.</b><span>Open Accounts and assign an investment account to Swing or Options.</span><button onClick={()=>navigate("Accounts")}>Configure an account →</button></div>}</section>}
          ...
          {tab==="Portfolio"&&<section className="portfolio-growth-opportunities"><header><span>LIVE MARKET SYNC · {advisorAccountName.toUpperCase()}</span><h2>{advisorPurpose} candidates and action alerts</h2><p>Every buy-more, hold, or trim result uses only this selected account’s holdings and position weights.</p></header><AutomaticMarketCopilot accessToken={accessToken} initialStrategy="long-term" ownedSymbols={ownedInvestmentSymbols} holdings={advisorHoldings} accountName={advisorAccountName} accountPurpose={advisorPurpose} selectedAccountType={advisorAccountType} onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action,accountId:advisorAccountId,accountName:advisorAccountName,accountPurpose:advisorPurpose}));navigate("Prepare Trade")}} onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} /></section>}
