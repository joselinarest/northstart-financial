      ...
      <div className="shell app-shell">
        ...
        <section className="content page-container" aria-busy={routeLoading}>
          ...
          {tab==="Accounts"&&<section className="connected-accounts card">
            ...
            {tab==="Accounts"&&swingHoldings.length>0&&<ConnectedHoldingsAnalysis marketOpen={marketPhase==="open"} holdings={swingHoldings} mode="swing" accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
            {tab==="Accounts"&&longTermHoldings.length>0&&<ConnectedHoldingsAnalysis marketOpen={marketPhase==="open"} holdings={longTermHoldings} mode="long-term" accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
            {tab==="Accounts"&&!!connectedFinance.holdings.length&&<section className="holdings-table"><h3>Investment holdings and growth tracking</h3>{connectedFinance.holdings.map(holding=>{const market=Number(holding.market_value_cents||0),hasCost=holding.cost_basis_cents!=null,cost=hasCost?Number(holding.cost_basis_cents):0,gain=market-cost;return <article key={`${holding.account_id}_${holding.ticker}_${holding.name}`}><span><b>{holding.ticker||"—"}</b><small>{holding.name}</small></span><span><small>Shares</small><b>{Number(holding.quantity||0).toLocaleString()}</b></span><span><small>Market value</small><b>${(market/100).toLocaleString()}</b><small>{holding.price_at?`${holding.source==="manual"?"Manual reference price":"Institution price"} ${new Date(holding.price_at).toLocaleString()}`:"Price timestamp unavailable"}</small></span><span className={hasCost?(gain>=0?"gain":"loss"):""}><small>{hasCost?"Gain / loss":"Purchase cost"}</small><b>{hasCost?`${gain>=0?"+":"−"}$${(Math.abs(gain)/100).toLocaleString()}`:"Not provided"}</b></span></article>})}</section>}
          </section>}
