...
export default function Home({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean } = {}) {
  ...
  const advisorSuggestions=useMemo(()=>{const held=new Set(advisorHoldings.map(holding=>String(holding.ticker||"").toUpperCase())),liquidOptionUnderlyings=new Set(["SPY","QQQ","AAPL","MSFT","NVDA","AMZN","META","GOOGL"]),isOptions=/option/.test(advisorPurposeKey),isSwing=/swing|trad/.test(advisorPurposeKey),isDividend=/dividend|income/.test(advisorPurposeKey),isRetirement=/retirement|401|ira|long.?term/.test(advisorPurposeKey);return investmentCatalog.filter(item=>item.category==="Stocks & ETFs"&&item.fit.includes(investorProfile)&&(!isOptions||liquidOptionUnderlyings.has(item.symbol))).map(item=>{const asset=marketAssets.find(value=>value.symbol===item.symbol),fair=modelFairValues[item.symbol],gap=asset&&fair?((asset.price-fair)/fair)*100:null,yieldPct=indicatedDividendYields[item.symbol]||0,purposeAdjustment=isDividend?(yieldPct>=1?14:yieldPct>0?4:-14):isRetirement?(/ETF|market/i.test(`${item.subcategory} ${item.metric}`)?12:0)-(item.risk==="High"?12:0):isOptions?(liquidOptionUnderlyings.has(item.symbol)?10:0):isSwing?(asset?8:0)-(item.risk==="High"?2:0):0,ownershipAdjustment=held.has(item.symbol)?3:0,adjusted=Math.max(0,Math.min(100,item.score+purposeAdjustment+ownershipAdjustment-(gap!==null&&gap>25?24:0)-(item.risk==="High"&&investorProfile!=="Active"?8:0))),accountWhy=isDividend?`${yieldPct>0?`${yieldPct.toFixed(2)}% indicated yield; `:"No regular yield confirmed; "}ranked for the account’s dividend-income purpose.`:isRetirement?`Ranked for ${advisorAccountType} diversification, long horizon, cost, and concentration control.`:isOptions?"Ranked as a liquid underlying to research before selecting any option contract.":isSwing?"Ranked for the swing account; entry, volume, stop, and reward/risk still must confirm.":"Ranked for this account’s general investment purpose and selected risk profile.";return {...item,gap,adjusted,accountWhy,isHeld:held.has(item.symbol)}}).sort((a,b)=>b.adjusted-a.adjusted).slice(0,5)},[investorProfile,advisorHoldings,advisorPurposeKey,advisorAccountType]);
  ...
  const connectedPortfolioAnalysis=useMemo(()=>{const labels={cash:"Cash & short-term",bonds:"Fixed income",diversified:"Core diversified",dividend:"Dividend quality",growth:"Growth / individual stocks"},values={cash:0,bonds:0,diversified:0,dividend:0,growth:0},classify=(holding:Record<string,any>):keyof typeof values=>{const symbol=String(holding.ticker||"").toUpperCase(),name=`${holding.name||""} ${holding.type||""}`.toUpperCase();if(/CASH|MONEY MARKET|SWEEP|TREASURY BILL/.test(`${symbol} ${name}`))return"cash";if(/BOND|FIXED INCOME|BND|AGG|TLT|IEF|SHY|MUNI/.test(`${symbol} ${name}`))return"bonds";if(/SCHD|VYM|DGRO|DVY|DIVIDEND/.test(`${symbol} ${name}`))return"dividend";if(/VTI|VOO|SPY|IVV|ITOT|VXUS|SPGP|FFLC|TOTAL MARKET|S&P 500|INDEX 500|FUND|ETF|TRUST/.test(`${symbol} ${name}`))return"diversified";return"growth"};const total=longTermHoldings.reduce((sum,item)=>sum+Number(item.market_value_cents||0),0)/100;const holdings=longTermHoldings.map(item=>{const ticker=String(item.ticker||"").toUpperCase(),name=String(item.name||"Unknown holding"),value=Number(item.market_value_cents||0)/100,bucket=classify(item),weight=total?value/total*100:0,isBroad=/^(VTI|VOO|SPY|IVV|ITOT|VXUS)$/.test(ticker)||/TOTAL MARKET|S&P 500|BROAD MARKET/.test(name.toUpperCase()),isFund=isBroad||/ETF|FUND|TRUST|INDEX/.test(name.toUpperCase())||/^(SPGP|FFLC|SCHD|VYM|DGRO|DVY|BND|AGG)$/.test(ticker);values[bucket]+=value;return{ticker,name,value,bucket,weight,isBroad,isFund}});const rows=(Object.keys(values) as Array<keyof typeof values>).map(key=>{const actual=total?values[key]/total*100:0,target=portfolioMix[key],delta=target-actual,status=Math.abs(delta)<=5?"balanced":delta>5?"underweight":"overweight",amount=Math.abs(delta)*total/100;return{key,label:labels[key],actual,target,delta,status,amount}});const concentration=holdings.sort((a,b)=>b.weight-a.weight).map(item=>{const bucketRow=rows.find(row=>row.key===item.bucket),overweight=Boolean(bucketRow&&bucketRow.delta<-5);if(item.weight<=10)return{...item,severity:"confirmed",verdict:"WITHIN CONCENTRATION LIMIT",finding:`At ${item.weight.toFixed(1)}% of the portfolio, this ${item.isFund?"fund":"individual position"} is below the 10% concentration review threshold. This confirms position size only—not valuation or future performance.`,action:"Keep it in the full live evidence review. Monitor fundamentals, trend, valuation and news; change it only if the investment thesis or portfolio role changes."};if(item.isBroad)return{...item,severity:overweight?"review":"confirmed",verdict:"DIVERSIFIED FUND · WEIGHT REVIEW",finding:`This fund spreads exposure across many companies, so ${item.weight.toFixed(1)}% is not the same risk as one company at that weight. The portfolio can still be dominated by the fund's U.S./equity market exposure.`,action:overweight?"Do not sell automatically. Direct new contributions to underweight categories first, then rebalance only if your saved target, time horizon, fees and tax rules support it.":"Broad diversification is confirmed. Continue monitoring allocation and overlap."};if(item.isFund)return{...item,severity:"review",verdict:"FUND · STYLE/OVERLAP REVIEW",finding:`This is a fund rather than one company, but ${item.weight.toFixed(1)}% can create style, sector, manager, or holdings overlap with the rest of the portfolio.`,action:"Run the live holding analysis, compare top holdings and expense ratio, and redirect contributions if overlap or category weight exceeds the plan. Sell only after restrictions and taxes are checked."};return{...item,severity:"risk",verdict:"INDIVIDUAL STOCK · CONCENTRATION RISK",finding:`One company's business and price risk represents ${item.weight.toFixed(1)}% of the portfolio. A company-specific loss could materially affect the retirement plan.`,action:"Use the live technical, fundamental, valuation and news review above. If the thesis fails or the weight exceeds your maximum, prepare a staged reduction; otherwise define a review date and invalidation level."}});return{total,rows,concentration}},[longTermHoldings,portfolioMix]);
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {["Market Intel","Scanner","Prepare Trade","Professional Charts"].includes(tab)&&<ConnectedHoldingsAnalysis holdings={swingHoldings} mode="swing" accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
          ...
          <section className="portfolio-builder card">
            {tab==="Portfolio"&&!!connectedFinance.holdings.length&&<section className="holdings-table analysis-holdings"><h3><small>ANALYSIS INPUT · CURRENT PORTFOLIO</small>Investment holdings, growth tracking, and evaluation</h3><HoldingPriceLimits holdings={longTermHoldings} accessToken={accessToken}/><ConnectedHoldingsAnalysis holdings={longTermHoldings} mode="long-term" accessToken={accessToken} horizon="2 years" onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} /></section>}
