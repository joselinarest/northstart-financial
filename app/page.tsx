...
export default function Home({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean } = {}) {
  ...
  const [familyAccessRequests,setFamilyAccessRequests]=useState<Array<Record<string,any>>>([]);
  ...
  useEffect(()=>{if(initialTab!=="Settings"||!signedIn)return;fetch("/api/security/access-requests",{headers:financeHeaders(),cache:"no-store"}).then(async response=>response.ok?response.json():null).then(data=>{if(data?.requests)setFamilyAccessRequests(data.requests)}).catch(()=>undefined)},[initialTab,signedIn,accessToken]);
  const reviewFamilyAccess=async(requestId:string,decision:"approved"|"denied")=>{setAccessReviewStatus(`${decision==="approved"?"Approving":"Denying"} request…`);const response=await fetch("/api/security/access-requests",{method:"PATCH",headers:financeHeaders(),body:JSON.stringify({id:requestId,decision})}),data=await apiPayload(response);if(!response.ok){setAccessReviewStatus(data.error||"Unable to review request");return}setFamilyAccessRequests(current=>current.map(item=>item.id===requestId?{...item,status:decision}:item));setAccessReviewStatus(`✓ Access ${decision}.`) };
    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {tab==="Daily Action Plan"&&<section className="daily-plan-intro card"><header><span>POST-CLOSE → NEXT SESSION → LIVE UPDATE</span><h2>One ranked plan that changes with the evidence</h2></header><div><article><b>After market close</b><p>Build the next-session list from completed daily bars, Weekly/Daily structure, volume, support, resistance, news, and risk.</p></article><article><b>At your decision window</b><p>Recheck gaps, opening range, relative volume, index/sector confirmation, and invalidate stale setups.</p></article><article><b>During market hours</b><p>Refresh every {intradayRefreshMinutes} minutes and promote only candidates whose trigger, liquidity, and reward/risk still qualify.</p></article></div><footer><b>Review alarm:</b> {decisionAlarmEnabled?`ON · ${decisionTime} New York / ${decisionTimeLabel}`:"OFF · configure it in Settings"}. Northstar proposes; you decide.</footer></section>}
          ...
          {["Daily Action Plan","Scanner","Market Intel","Prepare Trade"].includes(tab)&&<AutomaticMarketCopilot accessToken={accessToken} onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action}));navigate("Prepare Trade")}} onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />}
          ...
          <section className="connected-accounts card">
            ...
            {investmentAccounts.map(account=><InvestmentAccountProfile key={`profile_${account.id}`} account={account} holdings={connectedFinance.holdings.filter(holding=>holding.account_id===account.id)} onSave={updateInvestmentAccount}/>)}
          ...
          <section className="growth-finder card" id="growth-finder">
            ...
            <AutomaticMarketCopilot accessToken={accessToken} initialStrategy="long-term" onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action}));navigate("Prepare Trade")}} onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />
          </section>
          ...
          <section className="household card" id="household">
            ...
            <div className="members">
              ...
              {householdAccess?.invitations.map(invite=>{const status=invite.status==="pending"&&new Date(invite.expires_at)<=new Date()?"expired":invite.status;return <span key={invite.id} className={`invitation-${status}`}><i className="avatar gold-bg">{status==="accepted"?"✓":"?"}</i><b>{invite.email}<small>{invite.role.replaceAll("_"," ")} · {status==="accepted"&&invite.accepted_at?`accepted ${new Date(invite.accepted_at).toLocaleString()}`:status==="pending"?`expires ${new Date(invite.expires_at).toLocaleDateString()}`:status}</small></b><em>{status.toUpperCase()}</em>{status==="pending"&&<div className="invitation-actions"><button type="button" disabled={inviteNotice.includes("resending")} onClick={()=>createInvitation(invite.email,invite.role,true)}>↻ Resend</button><button className="remove-access" type="button" onClick={()=>cancelInvitation(invite.id)}>Cancel invitation</button></div>}</span>})}
            </div>
          ...
          <section className="attention card" id="attention-settings">
            ...
            <div className="attention-grid">
              {!!familyAccessRequests.length&&<div className="setting-block family-access-approval"><b>Family account approvals</b><p>Uninvited users cannot access any household or financial data until an administrator approves them. Invited users still receive only their assigned household role.</p>{familyAccessRequests.map(request=><article key={request.id}><span><b>{request.display_name}</b><small>{request.email} · requested {new Date(request.requested_at).toLocaleString()}</small></span><em>{String(request.status).toUpperCase()}</em>{request.status==="pending"&&<div><button onClick={()=>reviewFamilyAccess(String(request.id),"approved")}>Approve isolated account</button><button className="deny" onClick={()=>reviewFamilyAccess(String(request.id),"denied")}>Deny</button></div>}</article>)}{accessReviewStatus&&<small>{accessReviewStatus}</small>}</div>}
