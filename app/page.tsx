...
import DividendGrowthFinder from "@/app/dividend-growth-finder";
...
export default function Home({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean } = {}) {
  ...
  const pathByTab: Record<string,string> = { Dashboard:"dashboard", Accounts:"accounts", "Market Intel":"markets", Portfolio:"portfolio", "Professional Charts":"charts", "Market News":"market-news", "Growth Finder":"growth", "Dividend Growth":"dividend-growth", "Bills & cards":"cash-flow", Scanner:"opportunities", "Daily Action Plan":"daily-action-plan", Liabilities:"debt", Household:"household", "Ask Northstar":"assistant", "Prepare Trade":"planner", "Paper Simulator":"simulation", Journal:"journal", Learn:"academy", Settings:"settings", Help:"help" };
  ...
  const navigationGroups = [
    ...
    {name:"Long-Term Investing",items:[["Portfolio","◫"],["Growth Finder","↗"],["Dividend Growth","$"]]},
        ...
        <section className="content">
          ...
          <div className="hero" id="dashboard-top">
            <div>
              ...
              {investmentAccounts.length>0&&<label className="advisor-account-context"><span>ADVISOR ANALYSIS FOR</span><select value={advisorAccountId} onChange={event=>{setAdvisorAccountId(event.target.value);localStorage.setItem("northstar-advisor-account",event.target.value)}}>{investmentAccounts.map(account=><option value={String(account.id)} key={String(account.id)}>{account.nickname||account.official_name||account.name} · {account.investment_purpose||"Purpose not assigned"} · {account.subtype||"Investment"}</option>)}</select><small>{advisorAccount?`${advisorAccount.owner_name||"Household"} · ${advisorHoldings.length} holding${advisorHoldings.length===1?"":"s"} · ${advisorAccount.investment_purpose||"Set a purpose in Accounts"}`:"Select an investment account"}</small></label>}
            </div>
            <button className="primary" onClick={() => navigate("Ask Northstar")}>
              ✦ Analyze an idea
            </button>
          </div>
          {tab==="Dividend Growth"&&<DividendGrowthFinder accessToken={accessToken} onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action}));navigate("Prepare Trade")}} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />}
          ...
          {["Daily Action Plan","Scanner","Market Intel","Prepare Trade"].includes(tab)&&<AutomaticMarketCopilot accessToken={accessToken} initialStrategy={advisorStrategy} ownedSymbols={ownedInvestmentSymbols} holdings={advisorHoldings} accountName={advisorAccountName} accountPurpose={advisorPurpose} selectedAccountType={advisorAccountType} onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action,accountId:advisorAccountId,accountName:advisorAccountName,accountPurpose:advisorPurpose}));navigate("Prepare Trade")}} onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />}
          ...
          {["Market Intel","Scanner","Prepare Trade","Professional Charts"].includes(tab)&&advisorHoldings.length>0&&<ConnectedHoldingsAnalysis holdings={advisorHoldings} mode={advisorStrategy} accessToken={accessToken} onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} />}
