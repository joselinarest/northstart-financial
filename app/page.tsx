...
export default function Home({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean } = {}) {
  ...
  useEffect(()=>{if(initialTab!=="Market Intel"||!swingAccounts.length||swingAccounts.some(account=>String(account.id)===advisorAccountId))return;const next=String(swingAccounts[0].id);setAdvisorAccountId(next);localStorage.setItem("northstar-advisor-account",next)},[initialTab,swingAccounts,advisorAccountId]);
  ...
  const navigationGroups = [
    ...
    {name:"Swing Trade Decisions",items:[["Daily Action Plan","☀"],["Market Intel","✦"],["Prepare Trade","◎"]]},
        ...
        <section className="content">
          ...
          <div className="hero" id="dashboard-top">
            <div>
              ...
              {(tab==="Market Intel"?swingAccounts:investmentAccounts).length>0&&<label className="advisor-account-context"><span>{tab==="Market Intel"?"SWING / OPTIONS ANALYSIS FOR":"ADVISOR ANALYSIS FOR"}</span><select value={advisorAccountId} onChange={event=>{setAdvisorAccountId(event.target.value);localStorage.setItem("northstar-advisor-account",event.target.value)}}>{(tab==="Market Intel"?swingAccounts:investmentAccounts).map(account=><option value={String(account.id)} key={String(account.id)}>{account.nickname||account.official_name||account.name} · {account.investment_purpose||"Purpose not assigned"} · {account.subtype||"Investment"}</option>)}</select><small>{advisorAccount?`${advisorAccount.owner_name||"Household"} · ${advisorHoldings.length} holding${advisorHoldings.length===1?"":"s"} · ${advisorAccount.investment_purpose||"Set a purpose in Accounts"}`:"Select an investment account"}</small></label>}
            </div>
            <button className="primary" onClick={() => navigate("Ask Northstar")}>
              ✦ Analyze an idea
            </button>
          </div>
          {tab==="Market Intel"&&!swingAccounts.length&&<section className="purpose-empty card"><b>No Swing or Options investment account is configured.</b><span>Smart Opportunities is reserved for accounts whose purpose is Swing or Options. Open Accounts, set the account purpose, and return here.</span><button onClick={()=>navigate("Accounts")}>Configure account →</button></section>}
