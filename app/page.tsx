...
export default function Home({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean } = {}) {
  ...
  const swingDecisionTabs=["Daily Action Plan","Market Intel","Prepare Trade"],longTermInvestmentTabs=["Portfolio","Growth Finder"],isSwingDecisionPage=swingDecisionTabs.includes(initialTab),isLongTermInvestmentPage=longTermInvestmentTabs.includes(initialTab);
  ...
  const applyPortfolioPreset = () => {setPortfolioMix(portfolioGoal==="2–3 years"?{cash:35,bonds:40,diversified:20,dividend:5,growth:0}:portfolioGoal==="10+ years"?{cash:5,bonds:10,diversified:55,dividend:15,growth:15}:{cash:10,bonds:10,diversified:40,dividend:20,growth:20});if(connectedPortfolioValue>0)setPortfolioAmount(connectedPortfolioValue);setPortfolioNotice(`✓ Smart target generated for ${advisorAccountName}. Compare every current holding below; security-level proposals require live growth, market-cap, valuation, dividend, trend, overlap, and risk checks.`)};
  ...
  const navigationGroups = [
    ...
    {name:"Swing / Options Accounts",items:[["Daily Action Plan","☀"],["Market Intel","✦"],["Prepare Trade","◎"]]},
  ...
  const navigationLabels:Record<string,string>={"Market Intel":"Smart Opportunities","Prepare Trade":"Prepare Trade Plan","Professional Charts":"Chart Analysis",Journal:"Decision Journal",Learn:"Trading Academy","Paper Simulator":"Practice Simulator"};
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          <section className="portfolio-builder card">
            ...
            <div className="portfolio-setup expanded"><label>Strategy / horizon<select value={portfolioGoal} onChange={e=>setPortfolioGoal(e.target.value as "Swing"|"2–3 years"|"5 years"|"10+ years")}><option>2–3 years</option><option value="5 years">3–5 years</option><option>10+ years</option></select></label><label>Account type<select value={portfolioAccount} onChange={e=>setPortfolioAccount(e.target.value as typeof portfolioAccount)}><option>Taxable brokerage</option><option>401(k)</option><option>Traditional IRA</option><option>Roth IRA</option></select></label><label>Portfolio amount<div className="portfolio-money"><b>$</b><input type="number" min="0" step="500" value={portfolioAmount} onChange={e=>setPortfolioAmount(Math.max(0,+e.target.value))}/></div></label><div className="preset-actions"><button onClick={applyPortfolioPreset}>✦ Recalculate ideal portfolio</button><button onClick={applyCoreDividendGrowthPreset}>◎ Apply 40 / 30 / 30</button>{connectedPortfolioValue>0&&<button onClick={()=>{setPortfolioAmount(connectedPortfolioValue);setPortfolioNotice(`Using $${connectedPortfolioValue.toLocaleString(undefined,{maximumFractionDigits:0})} from synchronized holdings as the planning value.`)}}>↻ Use connected value</button>}</div></div>
