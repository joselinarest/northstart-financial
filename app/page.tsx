...
import PortfolioConcentrationReview from "@/app/portfolio-concentration-review";
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          <section className="portfolio-builder card">
            {tab==="Portfolio"&&!!connectedFinance.holdings.length&&<section className="holdings-table analysis-holdings"><h3><small>ANALYSIS INPUT · CURRENT PORTFOLIO</small>Investment holdings, growth tracking, and evaluation</h3>{connectedFinance.holdings.map(holding=>{const market=Number(holding.market_value_cents||0),hasCost=holding.cost_basis_cents!=null,cost=hasCost?Number(holding.cost_basis_cents):0,gain=market-cost;return <article key={`portfolio_${holding.account_id}_${holding.ticker}_${holding.name}`}><span><b>{holding.ticker||"—"}</b><small>{holding.name}</small></span><span><small>Shares</small><b>{Number(holding.quantity||0).toLocaleString()}</b></span><span><small>Market value</small><b>${(market/100).toLocaleString()}</b><small>{holding.price_at?`${holding.source==="manual"?"Manual reference price":"Institution price"} ${new Date(holding.price_at).toLocaleString()}`:"Price timestamp unavailable"}</small></span><span className={hasCost?(gain>=0?"gain":"loss"):""}><small>{hasCost?"Gain / loss":"Purchase cost"}</small><b>{hasCost?`${gain>=0?"+":"−"}$${(Math.abs(gain)/100).toLocaleString()}`:"Not provided"}</b></span></article>})}<ConnectedHoldingsAnalysis holdings={longTermHoldings} mode="long-term" onOpen={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);navigate("Professional Charts")}} /></section>}
