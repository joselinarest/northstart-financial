    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {tab==="Daily Action Plan"&&<section className="daily-plan-intro card"><header><span>REAL MARKET DATA · POST-CLOSE → NEXT SESSION → LIVE UPDATE</span><h2>One ranked plan that changes with current evidence</h2></header><div><article><b>After market close</b><p>Build the next-session list from completed provider bars, Weekly/Daily structure, volume, support, resistance, current fundamentals, news, and risk.</p></article><article><b>At your decision window</b><p>Recheck current gaps, opening range, relative volume, index/sector confirmation, and invalidate stale setups.</p></article><article><b>During market hours</b><p>Refresh provider data every {intradayRefreshMinutes} minutes and promote only candidates whose trigger, liquidity, and reward/risk still qualify.</p></article></div><footer><b>Live decision workspace:</b> values and timestamps come from the connected providers shown below. {decisionAlarmEnabled?`Review alarm ON · ${decisionTime} New York / ${decisionTimeLabel}`:"Review alarm OFF · configure it in Settings"}. Northstar proposes; you decide.</footer></section>}
