...
export default function Home({ initialTab = "Dashboard", initialInvestmentId, focusInvestmentAnalysis = false }: { initialTab?: string; initialInvestmentId?: string; focusInvestmentAnalysis?: boolean } = {}) {
  ...
  const navigationGroups = [
    ...
    {name:"Swing Trade Decisions",items:[["Market Intel","✦"],["Scanner","⌕"],["Prepare Trade","◎"]]},
  ...
  return (
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      <RealtimeSync accessToken={accessToken} onStatus={setRealtimeStatus} onEvent={()=>setRealtimeTick(value=>value+1)} />
      <header>
        <div className="brand">
          <span>N</span>
          <b>NORTHSTAR</b>
        ...
        <section className="content">
          ...
          <div className="hero" id="dashboard-top">
            <div>
              ...
              <h1>{tab==="Household"?"Your household financial command center.":tab==="Learn"?"Learn investing from foundation to professional practice.":tab==="Paper Simulator"?"Practice decisions without risking real money.":"Your professional trading copilot."}</h1>
              <p>
          ...
          {["Dashboard","Scanner","Market Intel","Prepare Trade"].includes(tab)&&<AutomaticMarketCopilot onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />}
          ...
          <section className="growth-finder card" id="growth-finder">
            ...
            <AutomaticMarketCopilot initialStrategy="long-term" onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />
          </section>
