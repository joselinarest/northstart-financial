    ...
    <main className={`workspace-view page-${(pathByTab[tab] || "dashboard").replace(/[^a-z-]/g, "")} ${focusInvestmentAnalysis?"page-research-detail":""}`}>
      ...
      <div className="shell">
        ...
        <section className="content">
          ...
          {tab==="Portfolio"&&<section className="portfolio-growth-opportunities"><header><span>LIVE MARKET SYNC · {advisorAccountName.toUpperCase()}</span><h2>{advisorPurpose} candidates and action alerts</h2><p>Every buy-more, hold, or trim result uses only this selected account’s holdings and position weights.</p></header><AutomaticMarketCopilot accessToken={accessToken} initialStrategy={advisorStrategy} ownedSymbols={ownedInvestmentSymbols} holdings={advisorHoldings} accountName={advisorAccountName} accountPurpose={advisorPurpose} selectedAccountType={advisorAccountType} onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action,accountId:advisorAccountId,accountName:advisorAccountName,accountPurpose:advisorPurpose}));navigate("Prepare Trade")}} onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} /></section>}
          ...
          <section className="growth-finder card" id="growth-finder">
            ...
            <AutomaticMarketCopilot accessToken={accessToken} ownedSymbols={ownedInvestmentSymbols} holdings={advisorHoldings} initialStrategy={advisorStrategy} accountName={advisorAccountName} accountPurpose={advisorPurpose} selectedAccountType={advisorAccountType} onPrepare={(symbol,action)=>{sessionStorage.setItem("northstar-chart-symbol",symbol);sessionStorage.setItem("northstar-prepared-action",JSON.stringify({symbol,action,accountId:advisorAccountId,accountName:advisorAccountName,accountPurpose:advisorPurpose}));navigate("Prepare Trade")}} onSelect={symbol=>{sessionStorage.setItem("northstar-chart-symbol",symbol);setChartSymbol(symbol);setMarketLookup(symbol);navigate("Professional Charts")}} />
          </section>
