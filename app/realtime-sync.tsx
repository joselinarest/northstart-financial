...
export default function RealtimeSync({accessToken,onStatus,onEvent}:{accessToken:string;onStatus:(status:string)=>void;onEvent:()=>void}){
 useEffect(()=>{
  ...
  if(!url){onStatus("MARKET 30s · PLAID ON SYNC");const poll=window.setInterval(()=>publish({type:"poll"}),30000);return()=>window.clearInterval(poll)}
  let delay=1000;
