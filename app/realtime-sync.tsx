...
export default function RealtimeSync({accessToken,onStatus,onEvent,refreshMinutes=1,marketOpen=false}:{accessToken:string;onStatus:(status:string)=>void;onEvent:()=>void;refreshMinutes?:number;marketOpen?:boolean}){
 useEffect(()=>{
  ...
  const publish=(detail:LiveEvent)=>{window.dispatchEvent(new CustomEvent("northstar:realtime",{detail}));if(marketOpen)onEvent();if(detail.type!=="alert"||!("Notification"in window)||Notification.permission!=="granted"||localStorage.getItem("northstar-push-enabled")==="false")return;new Notification(detail.title||"Northstar action review",{body:detail.explanation||"Material evidence changed. Open Northstar to review; no order was placed.",tag:detail.id||"northstar-alert"})};
  ...
  if(marketOpen)startPoll();else onStatus("MARKET CLOSED · MANUAL REFRESH");
