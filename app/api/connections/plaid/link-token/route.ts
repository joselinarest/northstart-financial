...
export async function POST(r:Request){
 try{
  ...
  const host=process.env.PLAID_ENV==="production"?"https://production.plaid.com":process.env.PLAID_ENV==="development"?"https://development.plaid.com":"https://sandbox.plaid.com",configured=[...new Set((process.env.PLAID_PRODUCTS||"transactions").split(",").map(value=>value.trim()).filter(value=>validProducts.has(value)))],primaryProduct=configured.includes("transactions")?"transactions":configured[0]||"transactions",products=[primaryProduct],optionalProducts=configured.filter(value=>value!==primaryProduct&&["auth","identity","investments","liabilities","signal"].includes(value));
