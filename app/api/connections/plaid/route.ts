  ...
  try {
    ...
    await db.transaction(async tx => {
      ...
      if(body.investmentPurpose==='Mixed'){
        await tx.prepare(`UPDATE account_risk_config SET custom_json=CASE WHEN custom_json IS NULL THEN NULL ELSE custom_json||'{"swingEnabled":true,"dayTradingEnabled":true,"longTermEnabled":false,"optionsEnabled":true}'::jsonb END,updated_at=CURRENT_TIMESTAMP WHERE account_id=?`).bind(body.accountId).run();
        await tx.prepare(`UPDATE investment_account_settings SET policy_json=COALESCE(policy_json,'{}'::jsonb)||'{"allowedStrategies":["SWING","DAY_TRADE","OPTIONS"],"allowedInstruments":["STOCKS","ETFS","CALL","PUT"]}'::jsonb WHERE account_id=?`).bind(body.accountId).run();
      }
