import {plaidSecurityId} from '@/lib/plaid-security-identity';
...
export async function POST(request: Request) {
  try {
    ...
    if (investments) {
      ...
      const priorHoldings=await db.prepare("SELECT h.id,h.account_id,h.security_id,h.provider_security_id,h.quantity,h.cost_basis_cents,h.price_cents FROM holdings h JOIN accounts a ON a.id=h.account_id WHERE a.connection_id=? AND h.provider_security_id IS NOT NULL").bind(body.connectionId).all<Record<string,any>>(),priorByKey=new Map(priorHoldings.results.map(row=>[`${row.account_id}|${row.provider_security_id}`,row])),seen=new Set<string>();
      for(const security of investments.securities||[])securityIds.set(security.security_id,await plaidSecurityId(db,security));
      for (const holding of investments.holdings || []) {
        holdingCount++;
        const accountId=`plaid_${holding.account_id}`,key=`${accountId}|${holding.security_id}`,prior=priorByKey.get(key),quantity=Number(holding.quantity||0),cost=holding.cost_basis==null?null:cents(holding.cost_basis),price=cents(holding.institution_price);seen.add(`${accountId}|${securityIds.get(holding.security_id)}`);if(!prior)holdingsAdded++;else if(Number(prior.quantity)!==quantity||Number(prior.cost_basis_cents??0)!==Number(cost??0)||Number(prior.price_cents??0)!==price)holdingsChanged++;
        writes.push(
          db
            .prepare(
            ...
            .bind(
              ...
              securityIds.get(holding.security_id)||`plaid_sec_${holding.security_id}`,
      ...
      for(const prior of priorHoldings.results)if(!seen.has(`${prior.account_id}|${prior.security_id}`)){holdingsClosed++;writes.push(db.prepare("DELETE FROM holdings WHERE id=?").bind(prior.id))}
      if (writes.length) await db.batch(writes);
    ...
    if(investments){
      ...
      try{
        while(offset<total){const page=await plaid("/investments/transactions/get",token,{start_date:iso(start),end_date:iso(now),options:{count:500,offset}});total=Number(page.total_investment_transactions||0);const writes:DbStatement[]=[];
          for(const security of page.securities||[])securityIds.set(security.security_id,await plaidSecurityId(db,security));
