...
export async function POST(request:Request){
  try{
    ...
    const response=await fetch("https://api.openai.com/v1/responses",{method:"POST",headers:{Authorization:`Bearer ${key}`,"Content-Type":"application/json"},body:JSON.stringify({model:process.env.OPENAI_MODEL||"gpt-5",store:false,instructions:system,input:`User question:\n${question}\n\nVerified application context (may be incomplete):\n${JSON.stringify(body.context||{}).slice(0,10000)}`,max_output_tokens:1400})});
    ...
    return Response.json({answer,model:process.env.OPENAI_MODEL||"gpt-5",asOf:new Date().toISOString(),readOnly:true});
