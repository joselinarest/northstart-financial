...
export async function GET(request:Request){try{
 ...
 db.prepare("SELECT DISTINCT ON ((captured_at AT TIME ZONE 'UTC')::date) captured_at,market_value_cents::text,cash_cents::text,cost_basis_cents::text,source_freshness_json FROM portfolio_snapshots WHERE account_id=? AND household_id=? ORDER BY (captured_at AT TIME ZONE 'UTC')::date DESC,captured_at DESC LIMIT 1500").bind(accountId,householdId).all(),
 ...
 return Response.json({accountId,snapshots:snapshots.results.reverse(),transactions:transactions.results,historyLimit:1500,transactionLimit:1000},{headers:{'Cache-Control':'private, no-store'}});
