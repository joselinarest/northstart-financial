...
export async function GET(request:Request){
  ...
  const technicalCandidates=Object.entries(data.bars||{}).flatMap(([symbol,bars])=>{
    ...
    const setup=strategy==="long-term"?(fundSymbols.has(symbol)?"Diversified fund—fund-specific review required":"Company stage not assigned until fundamentals are verified"):last.c>high20?"Breakout—needs acceptance":last.c>=ema20&&last.c<=ema20*1.025?"Pullback near EMA20":last.c>sma50?"Uptrend—wait for entry quality":"Below SMA50—reclaim required";
    const action=score>=76?"Research now":score>=62?"Watch for confirmation":"Avoid / wait";
  ...
  if(strategy==="long-term"&&process.env.FINNHUB_API_KEY){
    ...
    candidates=await Promise.all(technicalCandidates.map(async candidate=>{try{
      ...
      const epsTTM=metrics.epsTTM??metrics.epsBasicExclExtraItemsTTM??null,cashFlowPerShare=metrics.cashFlowPerShareTTM??null,normalizedGrowth=[revenueGrowth3Y,epsGrowth3Y].filter((value):value is number=>value!==null&&Number.isFinite(value)).map(value=>Math.max(0,Math.min(30,value))),growthAnchor=normalizedGrowth.length?avg(normalizedGrowth):null,targetPe=growthAnchor===null?null:Math.max(14,Math.min(35,15+growthAnchor*.65)),earningsValue=epsTTM&&epsTTM>0&&targetPe?epsTTM*targetPe:null,targetCashFlowMultiple=growthAnchor===null?null:Math.max(12,Math.min(28,13+growthAnchor*.5)),cashFlowValue=cashFlowPerShare&&cashFlowPerShare>0&&targetCashFlowMultiple?cashFlowPerShare*targetCashFlowMultiple:null,fairValue=earningsValue&&cashFlowValue?(earningsValue*.65+cashFlowValue*.35):earningsValue||cashFlowValue,fairValueLow=fairValue?fairValue*.75:null,fairValueHigh=fairValue?fairValue*1.25:null,fairValueSource=fairValue?`Northstar earnings/cash-flow model (${targetPe?.toFixed(1)??"—"}x earnings and ${targetCashFlowMultiple?.toFixed(1)??"—"}x cash-flow assumptions; ±25% uncertainty range)`:"Unavailable — positive EPS or cash flow per share and growth inputs are missing";
      const classification=classifyLongTermCandidate(candidate.symbol,profile.marketCapitalization??null);
      return{...candidate,...classification,name:profile.name||candidate.symbol,score,tier,fundamentals:{revenueGrowth3Y,revenueGrowth5Y,epsGrowth3Y,epsGrowth5Y,pe,forwardPE,peg,grossMargin,operatingMargin,marketCap:profile.marketCapitalization??null},fairValue,fairValueLow,fairValueHigh,fairValueSource,fairValueAsOf:new Date().toISOString(),news:[],missingData:coverage<6,forecast:"Continuation is plausible only if multi-year growth, margins and cash generation persist without excessive valuation or dilution."};
