...
export async function POST(request:Request){
 try{
  ...
  const invite=await db.prepare("SELECT id,household_id,email,role,expires_at FROM household_invitations WHERE token_hash=? AND status='pending'").bind(await hash(body.token)).first<{id:string;household_id:string;email:string;role:string;expires_at:string}>();
  ...
  await db.batch([
   db.prepare("INSERT INTO household_members(household_id,user_id,role,status) VALUES(?,?,?,'active') ON CONFLICT(household_id,user_id) DO UPDATE SET role=excluded.role,status='active'").bind(invite.household_id,user.userId,invite.role),
   ...
   db.prepare("INSERT INTO audit_log(id,household_id,user_id,action,target_type,target_id) VALUES(?,?,?,?,?,?)").bind(id("audit"),invite.household_id,user.userId,"household.invitation.accepted","invitation",invite.id),
  ]);
  return Response.json({householdId:invite.household_id,role:invite.role,email:user.email,status:"accepted"});
