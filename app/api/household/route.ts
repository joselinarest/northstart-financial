...
export async function GET(r:Request){
 try{
  ...
  const [household,members,invitations,available]=await Promise.all([
   ...
   ["owner","co_owner"].includes(role)?db.prepare("SELECT id,email,role,status,expires_at,created_at,accepted_at,accepted_by FROM household_invitations WHERE household_id=? AND status='pending' ORDER BY created_at DESC LIMIT 50").bind(householdId).all():Promise.resolve({results:[]}),
