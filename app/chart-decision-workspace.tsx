...
export default function ChartDecisionWorkspace(props:Props){
  ...
  useEffect(()=>{let active=true;const load=async()=>{setLoading(true);try{const response=await fetch(`/api/market/research?symbol=${encodeURIComponent(props.symbol)}`),data=await response.json();if(active)setResearch(data)}catch{if(active)setResearch({status:"unavailable",error:"Company research is temporarily unavailable."})}finally{if(active)setLoading(false)}};load();const timer=props.marketOpen?setInterval(load,300000):null;return()=>{active=false;if(timer)clearInterval(timer)}},[props.symbol,props.marketOpen]);
