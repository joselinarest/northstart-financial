...
export default function SwingOptionsAdvisor({
  ...
  if(sessionOnly){const candidates=results.filter(item=>item.contract&&((item.status==='CANDIDATE'&&item.decision.action==='BUY_IF')||(item.status==='RESEARCH'&&item.researchQualified)));return <section className="border-t border-line p-4" aria-label="Session options"><h3>Options</h3>{notice?<p role="status">Options analysis unavailable: {notice}</p>:!accountPolicy?<p>Loading options analysis…</p>:!candidates.length?<p>{accountPolicy.optionsEnabled?(results.length?'No option order is ready. '+(results[0].decision?.interpretation||'Research is awaiting confirmation.'):'Options research has not returned a completed decision for this account. Open options research to inspect scan progress.'):'Options are disabled for this account.'}</p>:candidates.map(item=><article className={"my-3! min-w-0! rounded-xl! border! border-l-4! p-4! [overflow-wrap:anywhere]! "+actionTone(item.contract.type).classes} key={item.contract.contractSymbol}><b>{actionTone(item.contract.type).icon} {item.underlying} · {item.contract.type} {item.status==='RESEARCH'?'NEXT-OPEN WATCH':'IF CONFIRMED'}</b><p>{item.contract.strike} strike · {item.contract.expiration} · {item.contract.dte} days</p><OptionBrokerIdentity symbol={item.underlying} contract={item.contract} contracts={item.decision.shares} actionable={item.status==='CANDIDATE'}/>{item.status==='RESEARCH'&&<p className="rounded-lg! bg-sky-50! p-3! text-sky-900!">After-close research · Quote as of {item.quoteAsOf ? new Date(item.quoteAsOf).toLocaleString() : item.asOf}. Reference premium per contract: {money(item.contract.premium)}. No order approved; refresh quotes and confirm funding, sizing and setup at the next open.</p>}<p>{item.decision.interpretation}</p><p>Contracts: {item.decision.shares||"Sizing pending"} · Total premium: {item.decision.shares?money(item.contract.premium*item.decision.shares):"Not approved"} · Maximum premium risk: {item.decision.shares?money(item.contract.maxLoss*item.decision.shares):"Not approved"}</p><HoldingCostBadge symbol={item.underlying} currentPrice={item.underlyingPrice}/><details><summary className="cursor-pointer! py-2! font-semibold!">Strike, Greeks and volatility</summary><p>Break-even {money(item.contract.breakeven)} · Delta {item.contract.delta??"Unavailable"} · Gamma {item.contract.gamma??"Unavailable"} · Theta {item.contract.theta??"Unavailable"} · Vega {item.contract.vega??"Unavailable"}</p><p>IV {item.contract.iv==null?"Unavailable":(item.contract.iv*100).toFixed(1)+"%"} · Spread {item.contract.spreadPct.toFixed(1)}% · Volume {item.contract.volume} · Open interest {item.contract.openInterest??"Unavailable"}</p><p>Per contract, a 1 percentage-point IV decrease changes premium by approximately {item.contract.vega==null?"unavailable":money(-item.contract.vega*100)}; one day of Theta changes value by approximately {item.contract.theta==null?"unavailable":money(item.contract.theta*100)}. Local sensitivities, holding other inputs constant; not forecasts.</p><OptionsVolatilityPanel assessment={item.volatility}/></details><p><b>When:</b> Only after fresh price, confirmation and account risk checks pass. {item.decision.whatWouldChange.join(' · ')}</p></article>)}<a href="/workspace/options">Open options research →</a></section>}
