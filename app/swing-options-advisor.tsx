...
export default function SwingOptionsAdvisor({
  ...
  return <section id="options-advisor" className="option-contract-advisor swing-options-advisor">
    ...
    {results.length > 0 && <div className="swing-option-results">{results.filter(r=>outcomeFilter==='ALL'||r.decisionLabel===outcomeFilter).map((result, rank) => {
      if(!result.contract)return <article className="exact-contract option-recommendation no-trade" key={result.underlying+rank}><strong>{result.decisionLabel||"NO OPTION TRADE"}</strong><h3>{result.underlying} · {result.account.name}</h3><HoldingCostBadge symbol={result.underlying} accountId={accountId} currentPrice={result.underlyingPrice}/><p>{result.decision.interpretation}</p>{result.rationale.map((reason,i)=><p key={i}>{reason}</p>)}<OptionsVolatilityPanel assessment={result.volatility}/><b>What would change this decision?</b><ul>{result.decision.whatWouldChange.map((reason,i)=><li key={i}>{reason}</li>)}</ul><small>Evaluated {new Date(result.asOf).toLocaleString()} · No option order recommended</small></article>;
