...
export function InvestmentConnectionRecoveryActions({ institutionName, busy, retryable = true, onRetry, onViewIssue, onAddManual }: BusyProps & { institutionName: string; retryable?: boolean; onRetry: () => void; onViewIssue: () => void; onAddManual: () => void }) {
  return <div className="investment-recovery-actions"><button type="button" disabled={busy || !retryable} onClick={onRetry}>{retryable ? `Retry ${institutionName}` : `${institutionName} connection unavailable`}</button><button type="button" onClick={onViewIssue}>View connection issue</button><button type="button" onClick={onAddManual}>Add {institutionName} manually</button></div>;
}

export function InvestmentConnectionError({ institutionName, errorCode, supportReference, message, occurredAt, retryable = true, requiresProviderRegistration = false, busy, onRetry, onAddManual }: BusyProps & { institutionName: string; errorCode?: string; supportReference?: string; message?: string; occurredAt?: string; retryable?: boolean; requiresProviderRegistration?: boolean; onRetry: () => void; onAddManual: () => void }) {
  const [expanded, setExpanded] = useState(false);
  return <section className="investment-connection-error" aria-live="polite"><div><span>INVESTMENT CONNECTION RECOVERY</span><h3>{institutionName} connection issue</h3><p>{requiresProviderRegistration ? `${institutionName} must be enabled for Northstar in the Plaid production dashboard before a live connection can work.` : `Plaid could not complete this ${institutionName} investment connection.`}</p></div><InvestmentConnectionRecoveryActions institutionName={institutionName} busy={busy} retryable={retryable} onRetry={onRetry} onViewIssue={() => setExpanded(value => !value)} onAddManual={onAddManual}/>{expanded && <div className="investment-connection-error-detail"><p>{message || "The provider did not complete the investment connection."}</p><small>{errorCode ? `Error ${errorCode} · ` : ""}{occurredAt ? `Last attempt ${new Date(occurredAt).toLocaleString()} · ` : ""}support reference {supportReference || "not available"}</small></div>}</section>;
}
