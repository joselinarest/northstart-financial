...
export default function PortfolioBalanceExplorer({ enabled=true, accountId, accessToken, rows, holdings, candidates, accountName, horizon, portfolioAmount, low, base, high }: { enabled?:boolean; accountId?:string; accessToken?:string; rows: Row[]; holdings: Holding[]; candidates: Candidate[]; accountName: string; horizon: string; portfolioAmount: number; low: number; base: number; high: number }) {
  ...
  const [saleData,setSaleData]=useState<{accountId:string;rows:any[];loading:boolean;error:string}>({accountId:'',rows:[],loading:false,error:''});
  useEffect(()=>{if(!enabled||!accountId)return;const controller=new AbortController();setSaleData({accountId,rows:[],loading:true,error:''});const householdId=localStorage.getItem('northstar-household-id');fetch('/api/recommendations?accountId='+encodeURIComponent(accountId),{credentials:'same-origin',cache:'no-store',signal:controller.signal,headers:{...(accessToken?{Authorization:'Bearer '+accessToken}:{}),...(householdId?{'X-Household-ID':householdId}:{})}}).then(async response=>{if(!response.ok)throw Error('Sell timing is unavailable; do not trade from the allocation estimate.');return response.json()}).then(body=>{if(!controller.signal.aborted)setSaleData({accountId,rows:body.recommendations||[],loading:false,error:''})}).catch(error=>{if(!controller.signal.aborted)setSaleData({accountId,rows:[],loading:false,error:error.message})});return()=>controller.abort()},[enabled,accountId,accessToken]);
  ...
  return <details className="portfolio-balance-explorer" open>
    ...
    {row && <section className={`balance-category-detail ${row.status}`}>
      ...
      <div className="balance-holdings"><b>Current holdings in this slice</b>{items.length ? items.map(item => {
        ...
        const direction = row.status === "overweight" ? "ABOVE ALLOCATION TARGET — NOT A SELL SIGNAL" : row.status === "balanced" ? "KEEP / MONITOR" : securityClearedForAdd ? "BUY / CONTRIBUTE REVIEW" : "AUTOMATIC SECURITY REVIEW";
        const actionClass = row.status === "overweight" ? "blocked" : row.status === "underweight" && !securityClearedForAdd ? "blocked" : row.status;
        return <span key={`${item.ticker}-${item.name}`} style={{display:"grid",gridTemplateColumns:"minmax(0,1fr) minmax(0,1fr)",gap:12}}><strong>{item.ticker || "—"}<small>{item.name}</small><small className={`holding-balance-action ${actionClass}`}>{direction}</small>{row.status === "underweight" && !securityClearedForAdd && <small className="holding-decision-reason">Northstar automatically runs the complete valuation, fundamentals, trend, and risk review below. The category gap alone cannot authorize buying this individual security.</small>}</strong><em>${item.value.toLocaleString(undefined, { maximumFractionDigits: 2 })}<small>{item.weight.toFixed(1)}% of account</small>{row.status !== "balanced" && securityClearedForAdd && <small className="holding-share-guide">Target-balance difference (not an order): ${adjustment.toLocaleString(undefined, { maximumFractionDigits: 0 })}{shares !== null ? ` · ${shares.toFixed(shares < 1 ? 2 : 1)} shares` : " · shares require current price"}</small>}</em>{row.status === "overweight"&&<SaleTimingReview allocationShares={shares??undefined} allocationPrice={item.price} symbol={item.ticker} accountId={accountId} recommendation={saleData.accountId===accountId?saleData.rows.find(r=>r.symbol===item.ticker):undefined} loading={saleData.loading} error={saleData.error}/>}</span>;
