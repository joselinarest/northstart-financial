...
export default function TradingDecisionBoard({data, portfolio, risk, status, marketOpen, onAnalyze, onRefresh, onSave, options}: {data:Row;portfolio:Row;risk:Row;status:Row|null;marketOpen:boolean;onAnalyze:()=>void;onRefresh:()=>void;onSave:(a:any)=>void;options?:ReactNode}) {
  ...
  return <div className="trading-board" data-testid="trading-board">
    ...
    <section aria-labelledby="trading-positions" className="rounded-xl border border-line bg-surface p-4"><h2 id="trading-positions">POSITIONS</h2><div className="trading-position-list">{(portfolio.holdings || []).map((h:Row)=>{
      ...
      return <article key={h.id} className="trading-position min-w-0 rounded-lg border border-line p-3"><b>{h.symbol} · {label}</b><p>{h.name}{h.quoteAsOf ? " · "+new Date(h.quoteAsOf).toLocaleString() : ""}</p><TradingFacts><span>Shares <b>{h.quantity}</b></span><span>Average cost <b>{money(h.averageCostCents)}</b></span><span>{h.quoteAsOf ? 'Latest quote' : 'Last recorded price'} <b>{money(h.priceCents)}</b></span><span>Position value <b>{money(h.marketValueCents)}</b></span><span>Gain / loss <b>{money(h.unrealizedPnlCents)}</b></span><span>Portfolio weight <b>{(h.weightBps/100).toFixed(2)}%</b></span></TradingFacts><p>{a?.why || (h.action==='REDUCE' ? 'Review concentration against account targets; trim only after thesis, tax and risk confirmation.' : 'Keep the position pending fresh account-specific analysis.')}</p>{(ready || prep) && <p>Next opportunity: {(ready || prep)?.priceCondition}</p>}<a href={`/workspace/research/${encodeURIComponent(h.symbol.toLowerCase())}?accountId=${encodeURIComponent(data.accountId)}`}>Research position →</a></article>;
