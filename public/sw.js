...
const VERSION = "northstar-pwa-v6";
...
self.addEventListener("install", event => event.waitUntil((LOCAL_DEVELOPMENT?Promise.resolve():caches.open(STATIC_CACHE).then(cache => cache.addAll(PRECACHE))).then(() => self.skipWaiting())));
