...
export async function handler(){
 ...
 const job=await db.prepare(`WITH candidate AS (SELECT id FROM background_jobs WHERE job_type IN ('OPTIONS_ACCOUNT_REVIEW','ACCOUNT_OPPORTUNITY_REVIEW') AND attempts<6 AND available_at<=CURRENT_TIMESTAMP AND (status IN ('QUEUED','FAILED') OR (status='RUNNING' AND locked_at<CURRENT_TIMESTAMP-INTERVAL '6 minutes')) ORDER BY CASE WHEN payload_json->>'manual'='true' THEN 0 ELSE 1 END,CASE WHEN attempts=0 THEN 0 ELSE 1 END,CASE WHEN created_at<CURRENT_TIMESTAMP-INTERVAL '2 hours' THEN 0 ELSE 1 END,COALESCE((payload_json->>'priority')::numeric,0)+EXTRACT(EPOCH FROM(CURRENT_TIMESTAMP-created_at))/300 DESC,created_at FOR UPDATE SKIP LOCKED LIMIT 1) UPDATE background_jobs j SET status='RUNNING',attempts=j.attempts+1,locked_at=CURRENT_TIMESTAMP,updated_at=CURRENT_TIMESTAMP FROM candidate c WHERE j.id=c.id RETURNING j.*`).first<Record<string,any>>();if(!job)break;
