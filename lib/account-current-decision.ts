...
export function accountCurrentDecision(data:Row,portfolio:Row,risk:Row,status:Row|null,now=Date.now(),search:Row|null=null){
 ...
 const researched=search?.candidates?.map((r:Row)=>r.decision_json).find((r:Row)=>r?.decision==='PREPARE'&&r.shares>0&&!(r.evidenceAgainst||[]).length&&!Object.keys(r.stageErrors||{}).length&&Number(r.expectancy?.score)>0);
