...
export async function accountRiskSettings(db:PostgresDatabase,householdId:string,accountId:string){
  const account=await db.prepare("SELECT a.id,s.maximum_risk_bps,CASE WHEN a.investment_purpose='Mixed' THEN 'MIXED' ELSE COALESCE(s.strategy_type,a.investment_purpose,'GROWTH_5_7') END strategy,COALESCE(s.available_cash_cents,a.available_balance_cents,0)::text cash,COALESCE((SELECT SUM(h.quantity*h.price_cents) FROM holdings h WHERE h.account_id=a.id),0)::text invested,COALESCE((SELECT MAX(h.quantity*h.price_cents) FROM holdings h WHERE h.account_id=a.id),0)::text largest FROM accounts a JOIN entities e ON e.id=a.entity_id LEFT JOIN investment_account_settings s ON s.account_id=a.id WHERE a.id=? AND e.household_id=? AND a.type='investment'").bind(accountId,householdId).first<{id:string;maximum_risk_bps:number|null;strategy:string;cash:string;invested:string;largest:string}>();
  ...
  const strategy=/MIXED/i.test(account.strategy)?'CUSTOM':/SWING/i.test(account.strategy)?'SWING':/RETIRE/i.test(account.strategy)?'RETIREMENT':/CHILD|CUSTODIAL/i.test(account.strategy)?'CHILD_GROWTH':/DIVIDEND/i.test(account.strategy)?'DIVIDEND_INCOME':'GROWTH_5_7';
  ...
  return {accountId,value,cash,recommended,effective};
}
