...
export async function accountRiskSettings(db:PostgresDatabase,householdId:string,accountId:string){
  ...
  const recommended=recommendedTradingPolicy({accountType:account.strategy,value,cash,largestPositionBps:value?cashMapping.largestInvestedCents/100/value*10000:0,liquid:false});
