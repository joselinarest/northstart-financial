...
export async function investmentCash(db:PostgresDatabase,householdId:string,accountId:string){
 ...
 return reconcileCash(Number(a.available_cash_cents??a.available_balance_cents??0),a.connection_id?Number(a.available_balance_cents||0):0,rows.map(h=>({symbol:String(h.symbol||(h.security_type==='cash'?'USD cash':'')),securityType:h.security_type,valueCents:Number(h.value_cents)})),cashSymbols(a.policy_json));
}
