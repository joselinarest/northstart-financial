...
export const PROVIDERS=["Plaid Banking","Plaid Investments","Market Data","Fundamentals","News","Options","Options Flow","Quant Data","Credit Provider","AI","Push","Email"] as const;
export type Provider=typeof PROVIDERS[number];
export async function providerConfiguration(){await loadRuntimeSecrets();const has=(...keys:string[])=>keys.every(k=>Boolean(process.env[k]));return {"Credit Provider":false,"Plaid Banking":has("PLAID_CLIENT_ID","PLAID_SECRET"),"Plaid Investments":has("PLAID_CLIENT_ID","PLAID_SECRET"),"Market Data":has("ALPACA_API_KEY","ALPACA_API_SECRET"),Fundamentals:has("FINNHUB_API_KEY"),News:has("FINNHUB_API_KEY"),Options:has("ALPACA_API_KEY","ALPACA_API_SECRET"),"Quant Data":has("QUANT_DATA_API_KEY"),"Options Flow":has("OPTIONS_FLOW_PROVIDER_URL","OPTIONS_FLOW_PROVIDER_TOKEN"),AI:has("OPENAI_API_KEY"),Push:has("VAPID_PUBLIC_KEY","VAPID_PRIVATE_KEY"),Email:has("RESEND_API_KEY","EMAIL_FROM")};}
export async function testProvider(provider:Provider){
  if(provider==='Credit Provider')return {status:'PROVIDER REQUIRED',error:'Select an approved consumer-permissioned credit provider with hosted identity and consent. No credit lookup is available.'};
