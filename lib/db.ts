...
function postgresSql(source: string) {
  ...
  sql = sql.replace(/\b([a-z][a-z0-9_]*\.(?:posted_at|created_at|updated_at|expires_at|evidence_as_of|point_at))\s*(>=|<=|>|<)\s*CURRENT_TIMESTAMP/gi,"NULLIF($1::text,'')::timestamptz $2 CURRENT_TIMESTAMP");
