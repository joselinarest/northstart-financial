...
export function buildOptionCandidates(contracts:OptionCandidateFacts[],underlying:DecisionOutput|null,cash:number,risk:number,catalystPass:boolean):DecisionCandidate[]{
  ...
  const options=contracts.filter(c=>c.volatility?.allowed===true&&(c.type==="CALL"?bullish:bearish)&&c.ask>0&&c.bid>0&&c.ask>=c.bid&&c.spreadPct<=8&&c.dte>=14&&c.volume>=10&&Number(c.openInterest)>0&&Number(c.impliedVolatility)>0&&[c.delta,c.gamma,c.theta,c.vega].every(v=>typeof v==="number"&&Number.isFinite(v))).slice(0,10).map(c=>{
