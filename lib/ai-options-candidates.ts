...
import type {assessOptionVolatility} from "./options-volatility";
export type OptionCandidateFacts={volatility?:ReturnType<typeof assessOptionVolatility>;contractSymbol:string;type:"CALL"|"PUT";ask:number;bid:number;dte:number;spreadPct:number;impliedVolatility:number|null;delta:number|null;gamma:number|null;theta:number|null;vega:number|null;volume:number;openInterest:number|null};
export function buildOptionCandidates(contracts:OptionCandidateFacts[],underlying:DecisionOutput|null,cash:number,risk:number,catalystPass:boolean):DecisionCandidate[]{
  ...
  const shares=(underlying.deterministicCandidates||[]).filter(c=>c.instrument==="SHARES"&&["BUY_NOW","BUY_IF","ADD"].includes(c.action)&&c.eligible&&c.cost<=cash&&c.entry!=null&&c.stop!=null&&c.entry>c.stop&&c.shares*(c.entry-c.stop)<=risk).map(c=>({...c,id:`shares-${c.id}`}));
  const options=contracts.filter(c=>c.volatility?.allowed===true&&(c.type==="CALL"?bullish:bearish)&&c.ask>0&&c.bid>0&&c.ask>=c.bid&&c.spreadPct<=8&&c.dte>=14&&c.dte<=30&&c.volume>=10&&Number(c.openInterest)>0&&Number(c.impliedVolatility)>0&&[c.delta,c.gamma,c.theta,c.vega].every(v=>typeof v==="number"&&Number.isFinite(v))).slice(0,10).map(c=>{
    const contracts=Math.floor(Math.min(cash,risk*(c.volatility?.riskMultiplier??0))/(c.ask*100)),cost=Math.round(contracts*c.ask*10000)/100;
