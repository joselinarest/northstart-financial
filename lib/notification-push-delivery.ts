...
export async function deliverPushDevices(db:PostgresDatabase,delivery:{id:string;household_id:string;user_id:string},send:(encrypted:string)=>Promise<void>){
  ...
  for(const subscription of subscriptions.results){
    ...
    catch(error){failed++;const status=Number((error as {statusCode?:number})?.statusCode);if([404,410].includes(status))await db.prepare("UPDATE push_subscriptions SET active=FALSE,updated_at=CURRENT_TIMESTAMP WHERE id=?").bind(subscription.id).run();}
  }
  if(failed)throw Error("PUSH_DELIVERY_FAILED");
}
