...
export function recommendedTradingPolicy(input:{accountType:string;value:number;cash:number;largestPositionBps:number;liquid:boolean}):TradingPolicy{
  const swing=/SWING/i.test(input.accountType),protectedAccount=/RETIRE|CHILD|COLLEGE|PRESERVATION/i.test(input.accountType),constrained=!input.liquid||input.value<=0||input.cash/input.value<.1||input.largestPositionBps>2500;
  return {version:"risk-1",label:"RECOMMENDED DEFAULTS",swingEnabled:swing,dayTradingEnabled:false,longTermEnabled:!swing,optionsEnabled:false,swingRiskBps:protectedAccount?15:constrained?25:50,dayRiskBps:constrained?10:20,combinedRiskBps:protectedAccount?100:constrained?100:200,maxPositionBps:protectedAccount?800:constrained?1000:1500,maxSectorBps:protectedAccount?2000:2500,dailyLossBps:constrained?50:100,weeklyDrawdownBps:protectedAccount?150:constrained?150:300,maxDayTrades:constrained?1:3,optionsRiskBps:protectedAccount?0:25,cashReserveBps:protectedAccount?1000:constrained?2000:1000,fridaySwing:"VERY_STRONG_ONLY",fridayConfidence:90,minimumConfidence:75,minimumRewardRisk:2,requireConfirmation:true,extendedHours:false,maxSwingDays:14,maxDayMinutes:360};
}
