...
export function recommendedTradingPolicy(input:{accountType:string;value:number;cash:number;largestPositionBps:number;liquid:boolean;volatilityPct?:number;maxDrawdownBps?:number}):TradingPolicy{
  const mixed=/MIXED/i.test(input.accountType),swing=/SWING/i.test(input.accountType)||mixed,protectedAccount=/RETIRE|CHILD|COLLEGE|PRESERVATION/i.test(input.accountType),constrained=!input.liquid||input.value<=0||input.cash/input.value<.1||input.largestPositionBps>2500;
