...
export async function runOptionsDiscovery(db:PostgresDatabase){
 ...
 for(const row of batch){
  ...
  if(stage==='CHAIN_QUALIFIED')await db.prepare(`INSERT INTO background_jobs(id,household_id,job_type,idempotency_key,payload_json) SELECT ?::text||a.id,e.household_id,'OPTIONS_ACCOUNT_REVIEW',?::text||a.id,jsonb_build_object('accountId',a.id,'symbol',?::text) FROM accounts a JOIN entities e ON e.id=a.entity_id JOIN account_risk_config r ON r.account_id=a.id WHERE a.hidden=0 AND a.type='investment' AND NOT EXISTS(SELECT 1 FROM background_jobs j WHERE j.job_type='OPTIONS_ACCOUNT_REVIEW' AND j.payload_json->>'accountId'=a.id AND j.payload_json->>'symbol'=? AND (j.status IN ('QUEUED','RUNNING') OR (j.status='FAILED' AND j.attempts<6))) ON CONFLICT(idempotency_key) DO NOTHING`).bind(id('option_review'),`${row.symbol}:${Math.floor(Date.now()/1800000)}:`,row.symbol,row.symbol).run();
 }
