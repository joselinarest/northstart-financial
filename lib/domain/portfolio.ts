...
const presets:Record<StrategyType,AllocationTarget>={
  LONG_TERM:{CORE:6000,QUALITY:2500,AGGRESSIVE:1000,INCOME:0,BONDS:0,CASH:500},
  LONG_TERM_ETF:{CORE:8000,QUALITY:0,AGGRESSIVE:0,INCOME:500,BONDS:1000,CASH:500},
  OPTIONS:{CORE:0,QUALITY:0,AGGRESSIVE:5000,INCOME:0,BONDS:0,CASH:5000},
...
export function calculatePortfolio(input:{holdings:HoldingInput[];cashCents:bigint;target:AllocationTarget;shareMode:"WHOLE"|"FRACTIONAL";maximumPositionBps:number;safeInvestmentCapacityCents:bigint;goalTargetCents?:bigint|null;horizonMonths?:number|null;annualContributionCents?:bigint;reservedReentryCents?:bigint;cashReserveBps?:number}){
  ...
  const positionRows=input.holdings.map(holding=>{const category=classifyHolding({securityType:holding.securityType,name:holding.name,ticker:holding.symbol}),weightBps=total>0n?Number(holding.marketValueCents*10000n/total):0,categoryGap=BigInt(categories.find(item=>item.category===category)!.gapCents),categoryValue=values[category],positionLimitCents=total*BigInt(input.maximumPositionBps)/10000n;let action:"BUY_MORE"|"HOLD"|"REDUCE"="HOLD",amount=0n;if(holding.marketValueCents>positionLimitCents){action="REDUCE";amount=holding.marketValueCents-positionLimitCents}else if(categoryGap>0n&&holding.marketValueCents<positionLimitCents){action="BUY_MORE";amount=[categoryGap,positionLimitCents-holding.marketValueCents,input.safeInvestmentCapacityCents].reduce((smallest,value)=>value<smallest?value:smallest)}else if(categoryGap<0n&&categoryValue>0n){action="REDUCE";amount=(-categoryGap)*holding.marketValueCents/categoryValue}const quantityScale=input.shareMode==="WHOLE"?1n:1000n,suggestedQuantity=holding.priceCents>0n?(amount*quantityScale/holding.priceCents):0n;return{quantity:holding.quantity,priceCents:holding.priceCents>0n?holding.priceCents.toString():null,averageCostCents:holding.costBasisCents!==null&&Number(holding.quantity)>0?String(Math.round(Number(holding.costBasisCents)/Number(holding.quantity))):null,id:holding.id,symbol:holding.symbol,name:holding.name,category,weightBps,marketValueCents:holding.marketValueCents.toString(),costBasisCents:holding.costBasisCents?.toString()||null,unrealizedPnlCents:holding.costBasisCents===null?null:(holding.marketValueCents-holding.costBasisCents).toString(),action,amountCents:amount.toString(),suggestedQuantity:(Number(suggestedQuantity)/Number(quantityScale)).toFixed(input.shareMode==="WHOLE"?0:3)}});
  const cashAllocation=allocateCash({cash:input.cashCents,safeCapacity:input.safeInvestmentCapacityCents,reserved:input.reservedReentryCents||0n,reserveBps:input.cashReserveBps??input.target.CASH,maximumPositionBps:input.maximumPositionBps,fractional:input.shareMode==="FRACTIONAL",targets:input.target,holdings:input.holdings.map(h=>({symbol:h.symbol,category:classifyHolding({securityType:h.securityType,name:h.name,ticker:h.symbol}),value:h.marketValueCents,price:h.priceCents}))});
  const assigned=new Set<string>();
  for(const row of positionRows){if(row.action!=="BUY_MORE")continue;const plan=cashAllocation.rows.find(p=>p.symbol===row.symbol);row.amountCents=assigned.has(row.symbol)?"0":plan?.estimatedCostCents||"0";row.suggestedQuantity=assigned.has(row.symbol)?"0":plan?.shares||"0";assigned.add(row.symbol);if(BigInt(row.amountCents)===0n)row.action="HOLD";}
  ...
  return{cashAllocation,totalValueCents:total.toString(),holdingValueCents:holdingValue.toString(),cashCents:input.cashCents.toString(),healthScore:health,healthInputs:{diversificationScore,concentrationScore:Math.round(concentrationScore),alignmentScore:Math.round(alignmentScore),cashScore,maxPositionBps:maxWeight,totalDriftBps:drift},categories,holdings:positionRows,projections,goalProgressBps};
}
