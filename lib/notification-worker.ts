...
async function processNotifications(request: Request,limits:{totalMs:number;jobMs:number}) {
  ...
  const claimJob = async()=>await db.prepare(`WITH claimable AS (
      ...
      ORDER BY CASE job_type WHEN 'AI_EVENT_REVIEW' THEN 0 WHEN 'PLAID_INVESTMENT_SYNC' THEN 1 WHEN 'PLAID_SYNC' THEN 1 WHEN 'NOTIFICATION_DELIVERY' THEN 2 WHEN 'ACCOUNT_INTELLIGENCE_LOOP' THEN 3 WHEN 'MARKET_INTELLIGENCE' THEN 4 ELSE 5 END,created_at FOR UPDATE SKIP LOCKED LIMIT 1
