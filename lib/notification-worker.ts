...
async function processNotifications(request: Request,limits:{totalMs:number;jobMs:number}) {
  ...
  const deliveries = await db.prepare(`WITH claimable AS (
      ...
      ) ORDER BY CASE WHEN a.severity IN ('action_now','critical') THEN 0 WHEN a.type='transaction' THEN 1 WHEN a.severity IN ('important','warning') THEN 2 ELSE 3 END,d.created_at
