...
export function category(type:string,title:string):NotificationCategory{if(type==="transaction")return /suspicious|fraud/i.test(title)?"Fraud/Security":"Household Finance";const text=type+" "+title;return /fraud|suspicious|security/i.test(text)?"Fraud/Security":/sync|system|provider.*fail/i.test(text)?"System":/option/i.test(text)?"Options":/buy|sell|trim|reentry|rebuy|recommendation|trade_lifecycle|stop|target/i.test(text)?"Trading Actions":/news|earnings|macro|market_intelligence/i.test(text)?"News":/portfolio|concentration|thesis/i.test(text)?"Portfolio":"Household Finance";}
