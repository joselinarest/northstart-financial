...
const exchangeFormatter=new Intl.DateTimeFormat('en-US',{timeZone:'America/New_York',year:'numeric',month:'2-digit',day:'2-digit',weekday:'short',hour:'2-digit',minute:'2-digit',hourCycle:'h23'});
export function exchangeDay(value:Date|string|number){
 const parts=Object.fromEntries(exchangeFormatter.formatToParts(new Date(value)).map(p=>[p.type,p.value]));
