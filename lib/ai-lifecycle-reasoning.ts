...
type Facts={checks:Record<string,unknown>;fundamentals:unknown;fundamentalAsOf?:string;newsAsOf?:string;news:unknown;valuation:unknown;technical:unknown;marketRegime:unknown;accountPolicy:unknown;researchComplete:boolean};
export function verifiedFundamentals(facts:Pick<Facts,"fundamentals"|"fundamentalAsOf">){
 const fund=facts.fundamentals as {metrics?:Record<string,unknown>}|null;
 const coverage=Object.values(fund?.metrics||{}).filter(v=>typeof v==='number'&&Number.isFinite(v)).length;
