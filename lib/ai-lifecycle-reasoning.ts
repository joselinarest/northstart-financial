...
export async function reasonLifecycle(db:PostgresDatabase,householdId:string,securityId:string,p:PositionState,a:Action,e:Evidence,risk:AccountRisk,facts:Facts,aiProvider?:AIProvider):Promise<DecisionOutput>{
  ...
  const evidence=[source("currentPrice",e.price,e.asOf,"PROVIDER"),source("fundamentals",facts.researchComplete?facts.fundamentals:null,facts.fundamentalAsOf||"","PROVIDER"),source("technical",facts.technical,e.asOf),source("patterns",facts.checks.patterns??[],e.asOf),source("portfolio",{shares:p.shares,averageCost:p.averageCost,value:risk.value}),source("accountCash",risk.cash),source("riskLimit",{maxPositionBps:risk.maxPositionBps,maxRiskBps:risk.maxRiskBps,reservedElsewhere:risk.reservedElsewhere}),source("news",facts.news,e.asOf,"PROVIDER"),source("valuation",facts.valuation,facts.fundamentalAsOf||""),source("marketRegime",facts.marketRegime,e.asOf,"PROVIDER")];
  ...
  const input={householdId,accountId:p.investmentAccountId,accountName:"Investment account",strategy:p.strategy==="LONG_TERM"?"LONG_TERM_SHARES" as const:p.strategy==="OPTIONS"?"OPTIONS" as const:"SWING_SHARES" as const,ticker:p.ticker,requestType:"TRADE_LIFECYCLE",features:{position:p,accountRisk:risk,accountPolicy:facts.accountPolicy,patterns:facts.checks.patterns??[],technical:e,deterministicAction:a},evidence,dataTimestamp:e.asOf,requiredFacts:[],candidates:lifecycleCandidates(p,a)};
