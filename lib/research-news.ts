...
export async function researchNews(db:PostgresDatabase,symbol:string,from:string,to:string){
 try{const result=await discoveryFinnhub(db,`/company-news?symbol=${encodeURIComponent(symbol)}&from=${from}&to=${to}`,300);if(!Array.isArray(result.data))throw Error('FINNHUB_NEWS_INVALID_RESPONSE');return {...result,provider:'FINNHUB'};}
 catch(original){
  ...
  if(saved)return {data:saved.payload_json,asOf:new Date(saved.fetched_at).toISOString(),provider:'ALPACA'};
