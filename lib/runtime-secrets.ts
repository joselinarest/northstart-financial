...
const permitted=new Set(["QUANT_DATA_API_KEY","OPTIONS_FLOW_PROVIDER_URL","OPTIONS_FLOW_PROVIDER_TOKEN","DATABASE_URL","DATABASE_POOL_MAX","DATABASE_SSL","DATABASE_SSL_REJECT_UNAUTHORIZED","TOKEN_ENCRYPTION_KEY","PLAID_CLIENT_ID","PLAID_SECRET","PLAID_ENV","PLAID_PRODUCTS","PLAID_COUNTRY_CODES","PLAID_WEBHOOK_URL","PLAID_REDIRECT_URI","PLAID_LINK_CUSTOMIZATION_NAME","OPENAI_API_KEY","OPENAI_MODEL","ALPACA_API_KEY","ALPACA_API_SECRET","ALPACA_DATA_FEED","ALPACA_CLOCK_BASE_URL","ALPACA_OPTIONS_FEED","FINNHUB_API_KEY","FRED_API_KEY","NEWSAPI_AI_KEY","RESEND_API_KEY","EMAIL_FROM","VAPID_PUBLIC_KEY","VAPID_PRIVATE_KEY","PUSH_DELIVERY_URL","PUSH_DELIVERY_TOKEN","CRON_SECRET","APP_URL"]);

export async function loadRuntimeSecrets(){
 const secretId=process.env.NORTHSTAR_SECRET_ID?.trim();
 if(!secretId)return;
 if(loading&&loadedAt&&Date.now()-loadedAt>=refreshMs){loading=null;loadedAt=0}
 ...
 try{await loading}catch(error){loading=null;loadedAt=0;throw new Error(`Secure server configuration could not be loaded: ${error instanceof Error?error.message:"AWS Secrets Manager error"}`)}
}
